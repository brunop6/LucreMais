...
-- Copiando estrutura para tabela lucremais.acao
CREATE TABLE IF NOT EXISTS `acao` (
  ...
  PRIMARY KEY (`id`),
  KEY `acao_ibfk_1` (`idPermissao`),
  CONSTRAINT `acao_ibfk_1` FOREIGN KEY (`idPermissao`) REFERENCES `permissao` (`id`) ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `permissao` (
  ...
  `dataAtualizacao` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `permissao_ibfk_1` (`idNivelUsuario`),
  KEY `permissao_ibfk_2` (`idMenu`),
  CONSTRAINT `permissao_ibfk_1` FOREIGN KEY (`idNivelUsuario`) REFERENCES `nivelusuario` (`id`) ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `usuario` (
  ...
  KEY `usuario_ibfk_1` (`idNivelUsuario`),
  CONSTRAINT `usuario_ibfk_1` FOREIGN KEY (`idNivelUsuario`) REFERENCES `nivelusuario` (`id`) ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4;
...
CREATE PROCEDURE `atualizaEstoque`(
	IN `param_idEstoque` INT,
	IN `param_idUsuario` INT,
	IN `quantidadeMovimento` DOUBLE,
	IN `param_observacao` VARCHAR(250),
	IN `tipo` CHAR(1)
)
BEGIN
